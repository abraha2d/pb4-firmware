...
async def recv_fw_data(client, topic, data, retained):
    ...
    print(f"ota.recv_fw_data: {Partition(Partition.RUNNING).get_next_update()}")

    FW_DATA.extend(data)
    print(f"main.recv_fw_data: Received {len(data)} bytes ({len(FW_DATA)} bytes total).")
...
async def recv_fw_hash(client, topic, data, retained):
    ...
    if fw_hash == data:
        print(f"main.recv_fw_hash: Hash valid. OTA update not implemented.")
        await client.publish(MQTT_TOPIC_OTA_FW_OK, "1", 1)
        status.app_state = status.APP_IDLE
    else:
        print(f"main.recv_fw_hash: Hash invalid! " +
...
async def recv_app_data(client, topic, data, retained):
    ...
    print(f"main.recv_app_data: Received {len(data)} bytes ({len(APP_DATA)} bytes total).")
    status.app_state = status.APP_RUNNING
...
async def recv_app_hash(client, topic, data, retained):
    ...
    if app_hash == data:
        print(f"main.recv_app_hash: Hash valid. OTA update not implemented.")
        await client.publish(MQTT_TOPIC_OTA_APP_OK, "1", 1)
        status.app_state = status.APP_IDLE
    else:
        print(f"main.recv_app_hash: Hash invalid! " +
